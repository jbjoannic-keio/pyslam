...
# update pose from transformation matrix
    ...
    def transform_points(self, points):
        with self._lock_pose:
            ...
            return (Rcw @ points.T +
                    tcw).T  # get points  w.r.t. camera frame  [Nx3]
    ...
    def is_visible(self, map_point):
        ...
        if np.dot(PO, map_point.get_normal()
...
class Frame(FrameBase):
    ...
    def __init__(self,
        ...
        if img is not None:
            ...
            if kps_data is None:
                ...
                kps_data = np.array(
                    [[x.pt[0], x.pt[1], x.octave, x.size, x.angle]
                     for x in self.kps],
                    dtype=np.float32)
                self.kps = kps_data[:, :2]
                self.octaves = np.uint32(
                    kps_data[:, 2])  #print('octaves: ', self.octaves)
    ...
    def replace_point_match(self, p, idx):
        self.points[
            idx] = p  # replacing is not critical (it does not create a 'None jump')
    ...
    def draw_feature_trails(self, img, kps_idxs, trail_max_length=9):
        ...
        with self._lock_features:
            ...
            for kp_idx in kps_idxs:
                ...
                radius = kDrawFeatureRadius[
                    self.octaves[kp_idx]]  # fake size for visualization
                ...
                if point is not None and not point.is_bad:
                    ...
                    for f, idx in p_frame_views[-trail_max_length:][::-1]:
                        if lfid is not None and lfid - 1 != f.id:
                            ...
                            break
                        pts.append(
                            tuple(map(lambda x: int(round(x)), f.kps[idx])))
                        lfid = f.id
                    if len(pts) > 1:
                        cv2.polylines(img,
